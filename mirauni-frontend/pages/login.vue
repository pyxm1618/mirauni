  ...
  <div class="min-h-[80vh] flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white border-2 border-indie-border shadow-brutal-lg p-8">
      ...
      <p class="text-xs text-gray-400 text-center mt-6">
        {{ $t('auth.agreement') }}
        <a href="#" class="text-indie-text underline">{{ $t('auth.userProtocol') }}</a>
        {{ $t('auth.and') }}
        <a href="#" class="text-indie-text underline">{{ $t('auth.privacyPolicy') }}</a>
      </p>
    </div>
  </div>
</template>
...
async function login() {
  ...
  console.log('[Login] Starting login process with phone:', phone.value, 'code:', code.value)

  error.value = ''
  isLoggingIn.value = true

  try {
    console.log('[Login] Calling loginWithCode...')
    // Race condition with a timeout
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('请求超时，请重试')), 15000)
    )

    await Promise.race([
      loginWithCode(phone.value, code.value),
      timeoutPromise
    ])
    
    console.log('[Login] loginWithCode success, redirecting...')
    // 登录成功后会自动跳转
  } catch (e: any) {
