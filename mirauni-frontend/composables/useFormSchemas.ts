import { z } from 'zod'
import { PROJECT_INDUSTRY_VALUES } from '~/types'
...
export const useFormSchemas = () => {
  ...
  const projectSchema = z.object({
    ...
    if (data.roles_needed.length < 1) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['roles_needed'], message: t('validation.project.roles.min') || 'Select at least one role' })
    }
    if (!data.work_mode) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['work_mode'], message: '请选择工作方式' })
    }
    if (!data.cooperation_type) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['cooperation_type'], message: '请选择合作方式' })
    }
  })
