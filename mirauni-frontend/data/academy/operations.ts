...
export const academyOperationsArticles: AcademyArticle[] = [
  {
    ...
    title: '隐私政策别从模板开始：先把网站到底收了什么数据画出来',
...
这恰好把顺序做反了。隐私说明应该描述你的真实数据处理，而不是让产品去适配一份模板。不同地区法律要求不同，正式合规问题需要结合运营主体和目标市场确认；但产品层面有一件事可以先做，而且几乎不会错：把数据流画出来。

## 从用户第一次打开网站开始列

浏览首页时有没有 analytics？有没有错误监控？服务端日志记录 IP 吗？有没有 Cookie、本地存储或设备标识？

注册时收什么：邮箱、手机号、Google 账号资料、头像？使用产品时又产生什么：输入内容、文件、历史记录、聊天、生成结果？这些数据是否会发给 OpenAI、Anthropic 或其他模型供应商？

付款时哪些信息由你保存，哪些只由支付服务商处理？退款记录保留多久？

把每个节点写成“数据 → 为什么需要 → 存在哪里 → 传给谁 → 留多久”。这张表比任何隐私模板更重要。

## 第三方服务不能只写一句“可能使用合作伙伴”

ICO 面向英国组织的指导要求在适用情况下说明个人数据的接收方或接收方类别、处理目的、保留期或判断标准等信息。

独立开发者最容易漏掉的是供应商事实。你可能没有主动“卖用户数据”，但仍然把数据交给了邮件服务、数据库托管、支付、分析、错误监控和 AI API。用户真正需要知道的是这些系统在流程里做什么，而不是一句抽象的“可信第三方”。
...
“我们绝不会保留任何数据”“所有信息立即删除”“采用最高级别安全措施”听起来让人放心，但如果日志保存 30 天、备份按周期覆盖、支付记录因为合法原因需要保存，这些话反而制造风险。
...
如果删除一个第三方 SDK，政策可以删掉；如果新增 GA4、Clarity 或新的模型服务，发布前就应该触发隐私复核。
...
Cookie 合规不应该从设计弹窗开始，而应该从“浏览器里到底写了什么”开始。下面以英国 ICO 的公开指导说明产品思路，具体地区要求可能不同。

## 先做一次 cookie inventory

打开生产站，用浏览器 DevTools 检查 Cookies、Local Storage、第三方网络请求。再把所有 SDK 列出来：认证、分析、客服、广告、A/B 测试、支付、嵌入视频。

对每一项写清用途、供应商、持续时间、是否在用户操作前就创建。很多团队做到这里才发现，所谓“我们只用了一个统计工具”其实顺带加载了几种存储和第三方请求。
...
更合理的是默认只加载严格必要能力；用户同意某类用途以后再初始化对应 Provider；撤回同意时更新状态，并尽可能停止后续非必要处理。

如果产品根本不需要非必要 cookie，最简单的 Banner 往往是不做复杂追踪。

## 第三方嵌入也要检查

YouTube、地图、聊天组件、社交嵌入可能在页面加载时向第三方发请求。不要只搜代码里有没有写 cookie。

一个完整 inventory 应该从真实浏览器网络行为出发。

新增一个增长工具时，把 cookie/privacy review 放进发布清单。Cookie 管理真正要解决的是：用户选择能不能控制实际数据行为。弹窗长什么样是第二步。`
  },
  {
    id: 'academy-operations-terms',
    slug: 'terms-of-service-should-match-real-business',
    title: '为什么不能照抄同行的 Terms：你的收费和退款规则不一样',
...
那家公司可能卖年费订阅，你卖一次性下载；它支持某种退款窗口，你的数字内容交付规则不同；它在美国经营，你在英国或中国；它可以随时停掉企业 API，你甚至没有 API。
...
退款页写 7 天，支付弹窗写“不退款”，客服又按另一套规则处理，同样会制造纠纷。

所以 Terms 不应该由一个孤立文档维护。收费模型、产品配置、退款政策、账户生命周期发生变化时，要有发布清单提醒同步检查法律页面。

## 数字服务尤其要把交付说清楚

GOV.UK 面向在线销售的指导要求经营者提供商品、服务或数字内容的描述、总价或计算方式、合同期限及滚动合同结束条件等信息。

这提醒独立开发者：不要只写按钮上的月费。用户需要知道买到什么、多久一次、怎样结束。

AI 服务还要说明更特殊的边界：结果是否保证准确？专业领域有什么限制？未成功生成时怎么处理次数或费用？这些规则如果影响购买决定，应该在购买前就能找到，而不是出问题后才拿 Terms 解释。

维护一张“商业事实表”：运营主体、产品、价格、计费方式、交付时点、自动续费、取消、退款、数据删除、客服邮箱、主要第三方。Privacy、Terms、Refund、Pricing 和代码都从这组事实核对。
...
条款最重要的品质不是长，而是和你真的怎么做生意一致。`
  },
  {
    ...
    title: '删除账号不是删一行 users：数据、订单和备份怎么办',
...
跨多个 Provider 删除很容易半成功。数据库删了，邮件服务 API 超时；对象存储成功，认证 Provider 失败。若按钮请求本身承担全部工作，用户看到一个 500，却不知道已经删到哪里。

更稳的设计是创建持久化 deletion job，记录每个阶段状态，失败可重试；在任务完成后再关闭登录能力或按设计冻结账户，避免删除过程中继续产生新数据。

## 匿名化有时比硬删除更适合历史记录

例如订单必须保留，但没必要一直关联用户昵称和头像，可以移除非必要个人字段，保留订单编号、金额、时间等必要事实。

匿名化必须是真的无法再合理识别个人，而不是把 email 换成 hash 就宣称完全匿名。

如果你无法回答“一个用户的数据到底在哪里”，说明隐私政策、备份和供应商管理也很可能不清楚。账号删除不是一条 SQL，而是一套数据生命周期。`
  },
  {
    id: 'academy-operations-subscription',
    slug: 'subscription-is-a-state-machine-not-a-boolean',
    title: '订阅不是一个 isPro：续费失败、取消和恢复都会让状态变复杂',
    ...
    content: `第一次做订阅，数据库里很容易出现一个字段：is_pro。
...
如果用户已经进入真正 canceled 的终态，再回来购买，才可能是新订阅。

这就是为什么一个 isPro 无法表达生命周期。
...
订阅上线前至少画出：创建、试用、激活、续费成功、续费失败、宽限、周期末取消、恢复、立即取消、退款、终止。再给每条箭头写触发来源：用户操作、后台操作、Provider webhook、定时任务。
