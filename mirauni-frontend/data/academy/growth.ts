...
export const academyGrowthArticles: AcademyArticle[] = [
  {
    ...
    title: 'SEO 到底在做什么：让一个真实需求落到一个最合适的页面',
...
问题是，搜索引擎真正要解决的并不是“哪个页面最努力做 SEO”，而是“哪个结果最适合当前这次搜索”。所以独立开发者做 SEO，第一步不是内容生产，而是需求映射。

## 先把关键词翻译成人话

用户搜“png to webp”，通常不是想读一篇图片格式历史。他希望上传文件并得到转换结果。

用户搜“what is webp”，则更可能在找解释。两个词都包含 webp，页面形态却完全不同。前者应该优先让工具完成任务，后者适合文章或文档。
...
Google 当前垃圾内容政策反对关键词堆砌、规模化低价值内容和以操纵排名为主要目的的行为。对独立开发者更实用的理解是：别为了覆盖词表制造页面库存。

## 技术 SEO 是让正确内容能被正常理解

内容对了，还要确保搜索引擎能访问。页面应该返回正确 HTTP 状态；重要内容不能被误设 noindex；站内链接要能到达；canonical、sitemap、重定向和多语言关系不能互相打架；移动端和真实用户体验也不能坏。
...
可能是工具更快、数据更新、解释更清楚、无需注册、免费层更合理、某个地区更适配，也可能是你拥有别人没有的一手数据。如果没有任何更好的地方，只靠“文章更长”通常不够。

每个准备做的页面只写五项：目标用户任务、主要搜索表达、页面类型、用户完成任务后的结果、与现有结果相比的理由。五项说不清，就先别写正文。
...
SEO 不是把搜索引擎当漏洞，而是把已经存在的搜索需求接到一个明确、可访问、值得留下的页面上。`
  },
  {
    ...
    title: '一个关键词该做一个页面还是多个页面？先看用户是不是在做同一件事',
...
所以第一层不是比较字面相似度，而是写出每个查询的完成状态。如果用户在两个查询下都希望得到同一个结果，优先考虑一个页面。

## 再看搜索结果是不是把它们当成同一个市场

手工搜索几个核心变体，比较前十结果。如果大部分 URL 高度重合，说明搜索系统和市场通常把它们视为接近的意图。一个强页面覆盖它们往往更自然。
...
Google 对 canonical 的说明指出，重复或非常相似页面会被聚类并选择代表页面。即使手工标了 canonical，最终选择仍是搜索系统综合判断。与其制造一堆让系统帮你去重的页面，不如一开始就让信息架构清楚。

## 同一个主题可以有多个页面，但任务必须不同

一个产品可以有工具页、方法页、定价页和案例页，它们围绕同一主题，却分别解决使用、理解、购买和信任问题。

长尾词也一样。“invoice template for freelancers”如果真的需要自由职业者特有字段、税务提醒和使用场景，可以有专页；如果只是把主页面标题加一句 for freelancers，就没有独立交付。

## URL 架构也要跟着意图，而不是跟着词表

一级页面应该对应稳定主题，不要今天为了一个长尾词建三层目录，明天又因为关键词变化迁移。

分类页如果只是把筛选参数全部开放给搜索引擎，会产生大量近似 URL。电商和目录站尤其要控制无限组合。

把候选关键词放进表格，只比较四列：用户任务、理想页面类型、现有 SERP 重合度、你能提供的独立价值。四列高度一致就合并；任务或交付明显不同才拆分。
...
先在 Search Console 用 URL Inspection 检查具体 URL，而不是不停用搜索命令猜。再确认 sitemap 已提交，重要页面从首页或其他可抓取页面有普通链接能到达。
...
这时问题从“收录”变成“竞争力”。目标词是不是和页面意图一致？前面的结果是不是更专业、更有权威、更直接完成任务？你的站点是不是一个完全没有历史的新域名？内部链接是否让重要页面显得孤立？
...
确认生产域名只有一个 canonical 版本；所有正式页面返回正确状态；预览环境 noindex；robots 没误拦；sitemap 只包含希望索引的 canonical URL；重要页面有站内链接；Search Console 验证并提交 sitemap；用 URL Inspection 抽查首页和核心页。

然后给搜索系统时间。

抓取解决“能不能看到”；索引解决“愿不愿意收”；排名解决“为什么是你”。先定位，再动手。`
  },
  {
...
广告和赞助链接并不是不能存在，但官方要求这类关系适当标记，例如 sponsored 或 nofollow，不要伪装成自然编辑推荐来传递排名信号。
...
先完成那些本来就合理的身份和产品入口：公司或作者资料、GitHub、真正相关的产品目录、平台生态、合作伙伴页面。

同时做值得被引用的资产：工具、原创数据、模板、研究、清楚的教程。没有任何可引用内容，外链工作会永远变成人工求链接。

判断一条外链值不值得做，可以问三个问题：如果 Google 明天完全不计算链接权重，这个入口还有没有真实用户价值？它和我的产品是否真的相关？为了得到它，我是否需要做自己不愿公开承认的操作？
...
外链不是数字收藏。它最好先是一条真实的互联网关系，然后才可能成为搜索信号。`
  },
  {
    ...
    title: 'Product Hunt、Reddit 冷启动：别把发布日当流量抽奖',
...
Product Hunt 用户愿意发现新产品，所以可以直接展示产品。Reddit 用户通常是为某个主题讨论而来，帖子应该围绕那个问题展开。Hacker News 更看重技术、创业和经验讨论。一份内容不能原封不动复制到所有平台。

渠道不是流量 API，每个社区都有自己的使用习惯。

## 发布后的 48 小时更重要

记录所有重复出现的问题。三个人在同一步卡住，就应该回产品看；反复有人问“和 X 有什么区别”，说明定位还没讲清；有人愿意用却不愿意付费，要分清是价值不够还是收费点太早。

发布前写下假设：谁会最感兴趣、他们会在哪一步感到价值、最可能反对什么、希望发生哪个转化。发布以后对照事实。

这样即使流量不大，仍然能得到一轮有效学习。冷启动不是寻找一个神奇入口，而是把产品一次次送到真正可能需要它的人面前。`
  },
]
